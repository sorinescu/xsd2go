...
func Convert(xsdPath, goModule, outputDir string, ignoredNamespaces []string, ignoredSubsts []string) error {
	fmt.Printf("Ignoring namespaces: %s\n", ignoredNamespaces)
	fmt.Printf("Processing '%s'\n", xsdPath)
	ws, err := xsd.NewWorkspace(goModule, outputDir, xsdPath, ignoredNamespaces, ignoredSubsts)
	if err != nil {
		return err
	}
